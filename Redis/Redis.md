...
-   解决办法：
    -   缓存空对象：当查询结果既不在db也不在缓存中，那么我们就把该key的value值设置成空对象保存到Redis中，那么在下次该缓存过期之前被访问那么就可以不用查询db了，直接返回该key所对应的空对象。
        -   缓存空对象的缺点：Redis缓存中存在大量的空对象，占用Redis内存。
    -   布隆过滤器

2.缓存击穿

-   概念：针对一条热点数据，高并发情况下去访问该热点数据，同时在访问的时刻该热点数据过期了。
    -   对于访问的热点数据，高并发情况下可能会造成db down掉。
...
- 是什么，能够解决什么问题
    ...
    - 针对上述问题，我们可以采用Hash一致性算法。首先我们有一个hash环的概念，0~2^32-1组成。我们知道每台Redis服务器有个唯一的IP地址，hash(ip)/2^32，得到的值就是每台服务器在hash环中的位置。
        ...
        ![](C:\Users\i337040\git\Java_Guide\Redis\resource\img\hash_consistency\hash_circle.png)

    - 存放图片，hash(图片id)/2^32找到其在hash环中的位置，**那么该数据应该存放在顺时针方向，最近的一个Redis服务器结点**。

        ![](C:\Users\i337040\git\Java_Guide\Redis\resource\img\hash_consistency\hash_circle2.png)
...
- Hash一致性算法会带来什么问题，怎么解决的
    ...
    - 怎么解决：创建虚拟节点，这样由于虚拟节点数的增多，那么数据就会尽可能均匀的分布在三台服务器上。
        ...
        ![](C:\Users\i337040\git\Java_Guide\Redis\resource\img\hash_consistency\virtual_nodes.png)
...
-   手动实现布隆过滤器算法思想：（借助Redis的Bitmap实现）
    ...
    -   通过bit数组长度和插入元素个数来确定hash函数个数
        ...
        -   判断元素是否存在：通过前面的key获取Bitmap，之后判断传过来的元素经过多个hash 函数运算，看返回的值是否存在0，如果存在那么表明该元素一定不存在，如果说全是1，那么只能表明该元素大概率存在。
